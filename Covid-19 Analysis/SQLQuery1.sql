...
SELECT location,MAX(CAST(total_deaths as bigint)) as deaths-- DISTINCT(CAST(total_cases as bigint))
FROM portfolio..covid_death
WHERE location is Not NULL and continent is not NULL
GROUP BY location
ORDER BY deaths desc

SELECT location,MAX(CAST(total_cases as bigint)) as cases-- DISTINCT(CAST(total_cases as bigint))
FROM portfolio..covid_death
WHERE location is Not NULL and continent is not NULL
GROUP BY location
ORDER BY cases desc

SELECT location,MAX(population) as abc--MAX(CAST(total_deaths as bigint)) as deaths-- DISTINCT(CAST(total_cases as bigint))
FROM portfolio..covid_death
WHERE location is Not NULL and continent is not NULL
GROUP BY location
ORDER BY location